...
\subsubsection{Συμβατότητα με το Νευρωνικό Δίκτυο}

Επειδή το regfile έχει σύγχρονη ανάγνωση, οι διευθύνσεις πρέπει να τεθούν \textbf{ένα κύκλο νωρίτερα}:

\begin{table}[H]
\centering
\caption{Pipelining διευθύνσεων RegFile στο FSM}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Τρέχουσα Κατάσταση} & \textbf{Διευθύνσεις που θέτουμε} & \textbf{Δεδομένα για} \\
\hline
...
S\_INPUT\_LAYER & weight\_3, weight\_4, bias\_3 & S\_OUTPUT\_LAYER \\
S\_OUTPUT\_LAYER & shift\_bias\_3 & S\_POSTPROCESS \\
\hline
\end{tabular}
\end{table}

Αυτή η τεχνική ``read-ahead'' εξασφαλίζει ότι τα δεδομένα είναι έτοιμα όταν τα χρειάζεται η ALU/MAC.
...
\begin{tikzpicture}[
    ...
    % --- Nodes (States) - 8 states ---
    \node[state, fill=blue!10, initial below] (s0) {S0 \\ DEACTIVATED};
    \node[state, fill=blue!10] (s1) [right=of s0] {S1 \\ LOADING};
    \node[state, fill=green!10] (s2) [below=of s0] {S2 \\ IDLE};
    \node[state] (s3) [right=of s2] {S3 \\ PRE-PROC};
    \node[state] (s4) [right=of s3] {S4 \\ INPUT \\ LAYER};
    \node[state, fill=orange!10] (s5a) [below=of s4] {S5a \\ OUTPUT \\ LAYER1};
    \node[state, fill=orange!10] (s5b) [left=of s5a] {S5b \\ OUTPUT \\ LAYER2};
    \node[state] (s6) [left=of s5b] {S6 \\ POST-PROC};

    % --- Transitions ---
    \path (s0) edge node {enable=1} (s1);
    \path (s1) edge [loop right] node {cnt $<$ 5} (s1);
    \path (s1) edge [bend left=30] node {done} (s2);
    \path (s2) edge [loop left] node {en=0} (s2);
    \path (s2) edge node {en=1} (s3);
    \path (s3) edge node {} (s4);
    \path (s4) edge node {} (s5a);
    \path (s5a) edge node {reg} (s5b);
    \path (s5b) edge node {} (s6);
    \path (s6) edge node {} (s2);

    % Overflow Handling (Red Dashed Lines)
    \draw [red, dashed] (s3) edge [bend left=60] node [above, sloped, font=\tiny] {OVF} (s2);
    \draw [red, dashed] (s4) edge [bend left=20] node [right, font=\tiny] {OVF} (s2);
    \draw [red, dashed] (s5a) edge [bend right=30] node [below, font=\tiny] {OVF} (s2);
    \draw [red, dashed] (s5b) edge [bend right=20] node [below, font=\tiny] {OVF} (s2);
    \draw [red, dashed] (s6) edge [bend right=10] node [left, font=\tiny] {OVF} (s2);

    % --- Legend ---
    \node [draw, rectangle, align=left, font=\tiny, below=0.5cm of s6] {
        \textcolor{blue}{Blue}: Init |
        \textcolor{green!50!black}{Green}: Ready |
        \textcolor{orange}{Orange}: Pipelined (break loop) |
        \textcolor{red}{Red}: Overflow
    };
...
\subsubsection{Σχεδιαστική Απόφαση: Διαχωρισμός Output Layer σε 2 Καταστάσεις}
...
\textbf{Συνέπεια:} Η υλοποίηση απαιτεί \textbf{8 καταστάσεις} αντί για 7, αλλά εξασφαλίζει σωστή λειτουργία χωρίς combinatorial loops.
...
\begin{itemize}
    \item \textbf{Αντιστοιχία με FSM diagram:} Η κωδικοποίηση S0=000, S1=001, S2=010, ... ακολουθεί ακριβώς τη ροή του διαγράμματος καταστάσεων
    ...
    \item \textbf{Εναλλακτικές:} Θα μπορούσε να χρησιμοποιηθεί one-hot encoding (7 bits) για ταχύτερη αποκωδικοποίηση ή Gray encoding για μείωση switching activity, αλλά η διαδοχική κωδικοποίηση είναι επαρκής για 7 καταστάσεις
\end{itemize}
...
\begin{enumerate}
    ...
    \item Η έξοδος τίθεται στο 0xFFFFFFFF (σύμφωνα με το reference model nn\_model 0xFFFFFFFF)
