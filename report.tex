...
\begin{tikzpicture}[
    ...
    node distance = 2.8cm and 3.2cm, % Slightly larger distances for uniform 2.5cm nodes
    state/.style = {
        circle, 
        draw, 
        minimum width = 3.5cm, % All states uniform size
        align=center, 
    ...
    % C. Overflow Handling (Red Dashed Lines)
    ...
    % --- Legend/Notes (Moved to bottom left) ---
    \node [draw, rectangle, align=left, font=\scriptsize,
            above left=-1cm and 9.5cm of s6.east % Positioning relative to bottom left state
          ] {
        \large
...
\end{tikzpicture}
\caption{Διάγραμμα καταστάσεων FSM του νευρωνικού}
\end{figure}
