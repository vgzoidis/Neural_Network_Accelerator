...
Ο encoder υλοποιήθηκε σε \textbf{structural Verilog} χρησιμοποιώντας βασικές πύλες (AND, OR, NOT, BUF). Οι λογικές εξισώσεις για κάθε bit του alu\_op προέκυψαν από τον πίνακα αλήθειας της εκφώνησης:
...
Οι απλοποιημένες εξισώσεις με χάρτες Karnaugh είναι:

\begin{align}
\text{alu\_op}[0] &= \text{btnd} \cdot (\text{btnl} + \overline{\text{btnr}}) \\
\text{alu\_op}[1] &= \text{btnl} \\
\text{alu\_op}[2] &= \text{btnr} + \text{btnd} \\
\text{alu\_op}[3] &= \text{btnr} \cdot \text{btnd}
\end{align}
