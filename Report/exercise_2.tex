...
Το calc module ως ακολουθιακό κύκλωμα με τις ακόλουθες θύρες εισόδου/εξόδου:
\begin{table}[H]
\centering
...
assign op2_extende ={{16{sw[15]}},sw};                   // sw -> 32-bit
\end{lstlisting}

\subsection{Encoder (calc\_enc.v)}
Ο encoder (calc enc.v) υλοποιήθηκε σε \textbf{structural Verilog} χρησιμοποιώντας βασικές πύλες (AND, OR, NOT, XOR) σύμφωνα με τα Σχήματα 2-5 της εκφώνησης. Οι λογικές εξισώσεις για κάθε bit του alu\_op είναι:
...
Η καλή λειτουργεία της αριθμομηχανής επιβεβαιώθηκε αποτυπώνοντας το έλεγχο της εκφώνησης στο calc\_tb.v:
\begin{table}[H]
    \centering
